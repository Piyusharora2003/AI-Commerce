...
exports.AddtoWishlist = asyncErrorHandler(async (req, res, next) => {
    ...
    if(!user) {
        return next(new ErrorHandler(`User doesn't exist with id: ${req.user.id}`, 404));
    }

    const newWishlist = [...user.wishlist,req.body.newItem];
...
exports.RemoveFromWishlist = asyncErrorHandler(async (req, res, next) => {
    ...
    const newWishlist = user.wishlist.filter(item=> (item.product) != (req.product));
    user.wishlist = newWishlist;
    await user.save()
